...
#define STOPPING_DISTANCE 20.0f             // TODO: This value has to be verified! Distance the winch needs to come to a complete stop (spool down then break) in m.
...
float calculateFeedForwardComponent(float commandedVelocity)
{
    ...
    return 0.008935879735684f * commandedVelocity + 0.654760350781171f;
}
//
// Utility functions
...
void setup()
{
    ...
    Serial.print(F("Created by: "));
    Serial.println(F(WINCH_CONTROL_AUTHOR));
    //
...
void loop()
{
        ...
        case WinchState::SHREDDING:
            // ???
            {
                ...
                if (desiredVelocity > commandedVelocity) {
                    //
                    // Check if we are under minimal velocity
                    if (commandedVelocity < MINIMAL_VELOCITY) {
                        commandedVelocity = MINIMAL_VELOCITY;
                    }
                    //
    ...
    updateEngineState();
    //
    // If engine turned off after the winch state changed, put the winch to
    // standby again
